...
export const Route = createFileRoute("/app/agent/training")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Complete mandatory compliance training and download certifications.",
      },
...
function AgentTrainingPage() {
  ...
  const [modules, setModules] = useState<TrainingModule[]>([
    {
      id: "mod-1",
      name: "RBI Fair Practice Code",
      duration: "18 min",
      requiredBy: "Completed",
      status: "Complete",
      description:
        "Mandatory codes governing transparency, rate disclosures, and grievance redressal channels.",
    },
    {
      id: "mod-2",
      name: "Data Consent and Privacy Gates",
      duration: "22 min",
      requiredBy: "Completed",
      status: "Complete",
      description:
        "Rules for collecting client OTPs. Covers mask-management protocols and data retention limits.",
    },
    {
      id: "mod-3",
      name: "Recognising Fraud and Impersonation",
      duration: "15 min",
      requiredBy: "31 Mar 2026",
      status: "Due",
      description:
        "Identify linked-device indicators, proxy attempts, and KYC document mismatches.",
    },
    {
      id: "mod-4",
      name: "Explaining APR and KFS to Borrowers",
      duration: "12 min",
      requiredBy: "30 Jun 2026",
      status: "Not started",
      description: "Guide to presenting Key Fact Statements without making non-compliant claims.",
    },
  ]);
    ...
    >
      <div className="space-y-6">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 text-xs">
          ...
          />
          <KpiCard
            label="Mandatory Quizzes Due"
            value={account === "new" ? "1" : "0"}
            tone={account === "new" ? "warning" : "neutral"}
          />
        ...
        {account === "new" && (
          <div className="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4 text-amber-900 text-xs">
            ...
            <div>
              ...
              <p className="mt-1">
                Your agent profile cannot be activated until Module 3 is completed and the
                assessment quiz is passed.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="grid gap-6 md:grid-cols-3">
          ...
          <div className="md:col-span-2 space-y-6">
                  ...
                  >
                    <div className="flex-1 pr-4 space-y-1">
                      ...
                      <p className="text-[10px] text-muted-foreground font-mono">
                        Duration: {m.duration} · Due: {m.requiredBy}
                      </p>
                    </div>
                    <span
                      className={`px-2 py-0.5 rounded text-[10px] font-semibold shrink-0 ${m.status === "Complete" ? "bg-emerald-50 text-emerald-700" : m.status === "Due" ? "bg-amber-50 text-amber-700" : "bg-neutral-100 text-muted-foreground"}`}
                    >
                      {m.status}
                    </span>
                  </div>
            ...
            <SectionCard
              title="Watch Again Library"
              description="Quick review links for completed courses"
            >
              <div className="grid gap-3 sm:grid-cols-2 text-xs">
                <button
                  type="button"
                  onClick={() => toast.success("FPC video loaded.")}
                  className="p-3 border border-border rounded-lg bg-card hover:border-primary text-left transition-colors"
                >
                ...
                <button
                  type="button"
                  onClick={() => toast.success("Consent video loaded.")}
                  className="p-3 border border-border rounded-lg bg-card hover:border-primary text-left transition-colors"
                >
                  <Play className="size-4 text-primary mb-2" />
                ...
                {account !== "new" ? (
                  <div className="space-y-2">
                    <Button
                      ...
                      onClick={() => toast.success("Certificate PDF downloaded.")}
                    >
                    ...
                    <Button
                      ...
                      variant="outline"
                      className="w-full"
                      onClick={() => toast.info("Triggered renewal course plan.")}
                    >
            ...
            <div className="w-full max-w-[500px] rounded-xl border border-border bg-card p-6 shadow-[var(--shadow-overlay)] text-center space-y-4">
              <div className="flex justify-between items-center border-b border-border pb-2 text-xs">
                <h3 className="font-bold text-sm text-foreground flex items-center gap-1.5">
                  <GraduationCap className="size-5 text-primary" /> Playing Module:{" "}
                  {activeVideo.name}
                </h3>
              </div>
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
                <div className="w-full max-w-[460px] rounded-xl border border-border bg-card p-6 shadow-xl space-y-4">
                  ...
                  <div className="pt-2 border-t border-border flex justify-end gap-2">
                    <Button size="sm" variant="ghost" onClick={() => setActiveQuiz(false)}>
                      Cancel
                    </Button>
                    <Button
                      size="sm"
