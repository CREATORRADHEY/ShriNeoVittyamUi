    ...
    >
      <div className="space-y-6">
        ...
        {scoreState === "not-generated" && (
          <SectionCard title="Compute Advisory Trust Score" description="Consent to calculate scoring parameters">
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                To build your score, we check cashflow data, transaction summaries, and utility dates. This helps thinner bureau profiles qualify for lower credit limits.
              </p>
              <div className="flex flex-wrap gap-2 pt-2">
                <Button size="sm" onClick={() => handleGenerateScore("available")}>
                  Generate SNV Trust Score
                </Button>
                <Button size="sm" variant="ghost" onClick={() => handleGenerateScore("insufficient-data")}>
        ...
        {scoreState === "available" && (
          <div className="grid gap-6 md:grid-cols-3">
            <div className="md:col-span-1 flex flex-col items-center justify-center rounded-xl border border-border bg-card p-6 text-center">
              <span className="text-xs text-muted-foreground font-semibold uppercase tracking-wider">SNV Trust Score</span>
              <span className="text-5xl font-mono font-bold text-foreground mt-3">68<span className="text-lg text-muted-foreground">/100</span></span>
              <span className="mt-2 text-xs font-semibold text-primary bg-primary-surface rounded px-2.5 py-0.5 border border-primary/20">MODERATE</span>
              <p className="mt-4 text-[10px] text-muted-foreground">Generated 10 Mar 2026</p>
              <Button size="xs" variant="outline" className="mt-5 w-full" onClick={() => setScoreState("not-generated")}>
                ...
                <div className="space-y-4 text-sm">
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Average Quarterly Balances are steady and show positive growth margins.</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-2.5">
                    <CheckCircle2 className="size-5 text-emerald-600 shrink-0 mt-0.5" />
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Monthly credit deposits match average salary/business disclosures.</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-2.5">
                    <CheckCircle2 className="size-5 text-emerald-600 shrink-0 mt-0.5" />
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Time spent at current location matches credit address queries.</p>
                    </div>
                  </div>
                </div>
              </SectionCard>

              <SectionCard title="Consent & Sharing Ledger" actions={
                sharingConsent ? (
                  <Button size="xs" variant="destructive" onClick={handleWithdrawConsent} className="flex items-center gap-1">
                    <EyeOff className="size-3" /> Block Sharing
                  </Button>
                ) : (
                  <Button size="xs" variant="default" onClick={handleGrantConsent} className="flex items-center gap-1">
                ...
                <div className="space-y-3">
                  <p className="text-xs text-muted-foreground">
                    Only lenders listed below can view your advisory SNV score. Withdrawal instantly hides details.
                  </p>
        ...
        {scoreState === "insufficient-data" && (
          <div className="flex flex-col items-center justify-center rounded-xl border border-border bg-card p-12 text-center">
            ...
            <p className="mt-2 max-w-[40ch] text-xs text-muted-foreground">
              We could not find sufficient digital banking or cashflow data to calculate an advisory score. Verify your linked accounts under profile options.
            </p>
