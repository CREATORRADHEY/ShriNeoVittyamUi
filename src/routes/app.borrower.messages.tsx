...
import { MessagesSquare, ShieldAlert, CheckCircle2, FileUp, Send, RefreshCw, X } from "lucide-react";
...
export const Route = createFileRoute("/app/borrower/messages")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Read and reply to messages from your lender, agent and support." },
...
interface ChatSession {
  ...
  history: { sender: "you" | "them"; text: string; time: string; status: "read" | "delivered" | "failed" }[];
}
  ...
  const [sessions, setSessions] = useState<ChatSession[]>([
    {
      ...
      history: [
        { sender: "them", text: "We noticed your Aadhaar address doesn't match the current electricity bill. Please upload a mismatch advice letter.", time: "12 Mar, 10:00", status: "read" }
      ]
    },
    {
      id: "agent",
      ...
      status: "Read",
      history: [
        { sender: "you", text: "I have uploaded the PAN card. Can you check?", time: "10 Mar, 11:00", status: "read" },
        { sender: "them", text: "Yes, PAN is verified. Excellent. I have advanced your file.", time: "10 Mar, 11:30", status: "read" }
      ]
    },
    {
      id: "support",
      ...
      status: "Resolved",
      history: [
        { sender: "them", text: "We have updated your phone registry records. The dispute is settled.", time: "04 Mar, 15:40", status: "read" }
      ]
    }
  ]);
      ...
      setSessions((prev) =>
        prev.map((s) => {
          if (s.id === activeSession.id) {
            return {
              ...s,
              history: s.history.map((h) => h.text === chatInput ? { ...h, status: "read" as const } : h)
            };
          }
          return s;
        })
        ...
        prev.map((s) => {
          if (s.id === activeSession.id) {
            return {
              ...
              history: [
                ...s.history,
                { sender: "them" as const, text: "Received your notes. The underwriting team is analyzing the details.", time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), status: "read" as const }
              ]
            };
          }
          return s;
      ...
      {isNewOrEmpty ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-border bg-card p-12 text-center">
          ...
          <p className="mt-2 max-w-[45ch] text-xs text-muted-foreground">
            When a lender, agent or our support team needs something from you, the conversation threads will appear here.
          </p>
        </div>
      ) : (
        <div className="grid gap-6 md:grid-cols-3">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <span className={`text-[9px] font-semibold px-2 py-0.5 rounded ${s.status === "Action needed" ? "bg-amber-50 text-amber-700 border border-amber-200" : s.status === "Resolved" ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-neutral-100 text-neutral-600 border border-neutral-200"}`}>
                        {s.status}
                      </span>
                    </div>
                  </div>
          ...
          <div className="md:col-span-2 rounded-xl border border-border bg-card overflow-hidden flex flex-col h-[550px]">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {activeSession.history.map((msg, index) => (
                <div key={index} className={`flex flex-col ${msg.sender === "you" ? "items-end" : "items-start"}`}>
                  <div className={`rounded-xl px-4 py-2.5 text-xs max-w-[85%] ${msg.sender === "you" ? "bg-primary text-primary-foreground rounded-tr-none" : "bg-neutral-100 text-foreground rounded-tl-none"}`}>
                    {msg.text}
                  </div>
                  <div className="flex items-center gap-1.5 mt-1 px-1 text-[9px] text-muted-foreground">
                    ...
                    {msg.sender === "you" && (
                      <>
                        <span>·</span>
                        {msg.status === "read" && <span className="text-emerald-700 font-semibold">Read</span>}
            ...
            <div className="p-3 border-t border-border bg-surface flex gap-2">
              <Button size="icon" variant="ghost" className="shrink-0" disabled={account === "suspended"} onClick={() => toast.info("Document Share vault triggered.")}>
                <FileUp className="size-5 text-muted-foreground" />
              </Button>
              <input
                type="text"
                disabled={account === "suspended"}
                placeholder={account === "suspended" ? "Messaging suspended" : "Type secure reply..."}
                value={chatInput}
              ...
              />
              <Button size="icon" disabled={account === "suspended"} onClick={handleSendMessage} className="shrink-0">
