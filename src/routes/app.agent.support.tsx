...
import { LifeBuoy, AlertTriangle, Phone, Mail, FileUp, Send, CheckCircle2, ChevronRight } from "lucide-react";
...
export const Route = createFileRoute("/app/agent/support")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Contact the agent desk, escalate files, and raise support tickets." },
...
function AgentSupportPage() {
  ...
  const [tickets, setTickets] = useState<AgentTicket[]>([
    { id: "SNV-AS-3312", subject: "Lender not responding on file SNV-24-118198", category: "Stuck File", raisedDate: "11 Mar 2026", status: "Escalated", slaHours: 24 },
    { id: "SNV-AS-3290", subject: "Commission entry missing for Rohit Sharma case", category: "Commission", raisedDate: "06 Mar 2026", status: "Open", slaHours: 48 }
  ]);
  ...
  const handleRaiseTicket = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const newTkt: AgentTicket = {
        ...
        subject: subject,
        category: category === "file" ? "Stuck File" : category === "commission" ? "Commission" : "General",
        raisedDate: "Today",
        status: "Open",
        ...
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-4 text-xs">
          <SectionCard title="Agent Call Helpdesk">
            ...
            <Button asChild size="sm" variant="outline" className="w-full mt-3">
              <a href="tel:1800000111"><Phone className="size-3.5 mr-1" /> 1800-000-111</a>
            </Button>
          </SectionCard>

          <SectionCard title="Email Agent Desk">
            <p className="text-muted-foreground">Submit files or logs directly. 24h SLA response.</p>
            <Button asChild size="sm" variant="outline" className="w-full mt-3">
              <a href="mailto:agentdesk@shrineocapital.com"><Mail className="size-3.5 mr-1" /> email desk</a>
            </Button>
          </SectionCard>

          <SectionCard title="Escalate Stuck File">
            <p className="text-muted-foreground">Force lender status refresh after 48h delay.</p>
            <Button size="sm" variant="outline" className="w-full mt-3" onClick={() => { setFormOpen(true); setCategory("file"); }}>
              Escalate File
            </Button>
          </SectionCard>

          <SectionCard title="Commission appeal">
            <p className="text-muted-foreground">Dispute clawbacks, payouts, or ledger adjustments.</p>
            <Button size="sm" variant="outline" className="w-full mt-3" onClick={() => { setFormOpen(true); setCategory("commission"); }}>
              Appeal Ledger
            </Button>
          </SectionCard>
        </div>
              ...
              <tbody>
                {tickets.map((t) => (
                  <tr key={t.id} className="border-b border-border">
                    ...
                    <td className="p-3 text-muted-foreground">{t.status === "Resolved" ? "Closed" : `${t.slaHours} Hours`}</td>
                    <td className="p-3">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-semibold ${t.status === "Escalated" ? "bg-amber-50 text-amber-700 border border-amber-200" : t.status === "Resolved" ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-neutral-100 text-muted-foreground border border-border"}`}>
                        {t.status}
                      </span>
                    </td>
                  </tr>
            ...
            <form onSubmit={handleRaiseTicket} className="w-full max-w-[450px] rounded-xl border border-border bg-card p-6 shadow-[var(--shadow-overlay)] space-y-4">
              <div className="flex items-center justify-between border-b border-border pb-2 text-xs">
                <h3 className="font-bold text-base text-foreground flex items-center gap-1.5">
                  ...
                  {category === "file" ? "Escalate Stuck File" : category === "commission" ? "File Commission Dispute" : "Raise Desk Ticket"}
                </h3>
              </div>

              <div className="space-y-3 text-xs">
                <div>
                  <label htmlFor="tkt-cat" className="block text-xs font-semibold text-muted-foreground mb-1">Category</label>
                  <select
                    id="tkt-cat"
                    value={category}
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={category === "file" ? "Sourced Application ID (e.g. SNV-24-118198)" : "Brief subject"}
                ...
                <div>
                  <label htmlFor="tkt-desc" className="block text-xs font-semibold text-muted-foreground mb-1">Description & Details</label>
                  <textarea
                    id="tkt-desc"
                    required
                    rows={4}
              ...
              <div className="pt-2 border-t border-border flex justify-end gap-2">
                ...
                {category === "file" && subject.startsWith("SNV") ? (
                  <Button type="button" size="sm" onClick={() => { handleEscalateStuckFile(subject); setFormOpen(false); }}>
                    Escalate Now
                  </Button>
                ) : (
