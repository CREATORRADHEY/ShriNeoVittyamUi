...
export const Route = createFileRoute("/app/borrower/action-centre")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Resolve pending lender tasks, upload documents and sign agreements.",
      },
  ...
  const handleUploadSim = () => {
    ...
    setTimeout(() => {
      ...
      const updated = requests.map((r) =>
        r.id === activeRequest.id ? { ...r, status: "Submitted" as const } : r,
      );
      ...
      {uploadState === "success" ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-border bg-card p-12 text-center">
          ...
          <div className="mt-4 w-full max-w-sm rounded-lg bg-surface border border-border p-4 text-left text-xs mx-auto space-y-2">
            ...
            <p>
              ...
              <span className="font-mono text-foreground font-semibold">{receiptRef}</span>
            </p>
            <p>
              <span className="text-muted-foreground">Timestamp:</span>{" "}
              <span className="text-foreground">{submitTime}</span>
            </p>
            <p>
              <span className="text-muted-foreground">Current Status:</span>{" "}
              <span className="inline-flex rounded-full bg-blue-50 border border-blue-200 px-2 py-0.5 text-[10px] font-semibold text-blue-700">
                SUBMITTED
              </span>
            </p>
            <p>
              <span className="text-muted-foreground">Audit Code:</span>{" "}
              <span className="text-foreground">Auditable record updated in platform timeline</span>
            </p>
          </div>
          <p className="mt-4 max-w-[45ch] text-xs text-muted-foreground">
            Your document has been sent to the lender underwriting team. We will notify you when
            they complete the review.
          </p>
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-border bg-card p-12 text-center">
          ...
          <p className="mt-2 max-w-[45ch] text-sm text-muted-foreground">
            No outstanding document requests or actions need your attention. We will notify you if a
            lender raises queries.
          </p>
          <Button asChild className="mt-6" variant="outline">
            <Link to="/app/borrower">Back to Dashboard</Link>
          </Button>
        ...
        <div className="space-y-6">
          <div className="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4 text-amber-900">
            ...
            <div>
              ...
              <p className="mt-1 text-xs">
                A participating lender has requested clarification. Submit the required file to
          ...
          <SectionCard
            ...
            actions={
              <span className="text-xs text-red-600 font-semibold bg-red-50 border border-red-200 rounded px-2.5 py-0.5">
                PENDING
              </span>
            }
          >
            <div className="space-y-4">
                ...
                <dl className="grid gap-2 sm:grid-cols-2">
                  <div>
                    ...
                    <dd className="font-semibold text-foreground mt-0.5">
                      SBI Digital Finance (Underwriting)
                    </dd>
                  </div>
                  <div>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  >
                    ...
                    <p className="mt-3 font-semibold text-foreground text-sm">
                      Verify via Account Aggregator
                    </p>
                    <p className="mt-1 text-xs text-muted-foreground">
                      Automated, secure, and password-free
                    </p>
                  </button>
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      Select statements file from your device
                    </p>
                  </button>
                </div>
              )}
              ...
              {method === "aa" && uploadState === "idle" && (
                <div className="rounded-lg border border-border p-4 space-y-4">
                  <h3 className="font-semibold text-sm text-foreground">
                    Account Aggregator (Consent Manager)
                  </h3>
                  <p className="text-xs text-muted-foreground">
                    We will fetch digital bank data via Finvu. Enter the OTP sent to your linked
                  ...
                  {!otpSent ? (
                    <Button
                      ...
                      onClick={() => {
                        setOtpSent(true);
                        toast.info("OTP sent to your linked mobile.");
                      }}
                    >
              ...
              {method === "upload" && uploadState === "idle" && (
                <div className="rounded-lg border border-border p-5 text-center space-y-3">
                  ...
                  <p className="text-xs text-muted-foreground">
                    PDF size limit 10MB. Must cover recent 6 months.
                  </p>
              ...
              {uploadState === "uploading" && (
                <div className="rounded-lg border border-border p-8 text-center space-y-3">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Establishing secure API channel to verification partner.
                  </p>
                </div>
              )}
            </div>
