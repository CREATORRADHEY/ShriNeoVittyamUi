...
import { Settings, CheckCircle2, AlertTriangle, ArrowRight, Save, Play, ShieldCheck, Activity } from "lucide-react";
...
function AdminTrustScorePage() {
  ...
  const [overrideLogs, setOverrideLogs] = useState<OverrideLog[]>([
    { timestamp: "12 Mar, 14:30", underwriter: "UW-994", applicationId: "SNV-24-118198", originalScore: "B", newScore: "A", reason: "Verified stable cash flows and validated utility deposit receipts manually" },
    { timestamp: "11 Mar, 10:15", underwriter: "UW-104", applicationId: "SNV-24-118165", originalScore: "C", newScore: "B", reason: "Co-applicant matched high-credit profile verification" }
  ]);
    ...
    >
      <div className="space-y-6 text-xs">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <KpiCard label="Population Drift (PSI)" value={populationDrift} hint="Threshold: 0.10" tone="success" />
          <KpiCard label="Bias/Fairness check" value={fairnessMetrics} hint="Disparate Impact: >0.80" tone="success" />
          <KpiCard label="Manual Score Overrides" value={`${overrideLogs.length} Registered`} hint="Last 30 days" tone="warning" />
        </div>

        {/* Workspace */}
        <div className="grid gap-6 md:grid-cols-3">
          <div className="md:col-span-2 space-y-6">
            <SectionCard title="Underwriter Override Audits" description="Every manual scoring override requires a documented business reason">
                  ...
                  <table className="w-full min-w-[600px] border-collapse text-left">
                    <thead>
                      <tr className="border-b border-border text-xs uppercase tracking-wide text-muted-foreground bg-surface">
                        ...
                        <th scope="col" className="p-3">Underwriter</th>
                        <th scope="col" className="p-3">Application ID</th>
                        <th scope="col" className="p-3">Original</th>
                        <th scope="col" className="p-3">Override</th>
                        <th scope="col" className="p-3">Justification Reason</th>
                      </tr>
                    </thead>
                    <tbody>
                      {overrideLogs.map((log, idx) => (
                        <tr key={idx} className="border-b border-border hover:bg-neutral-50">
                          ...
                          <td className="p-3"><StatusBadge tone="neutral">{log.originalScore}</StatusBadge></td>
                          <td className="p-3"><StatusBadge tone="success">{log.newScore}</StatusBadge></td>
                          <td className="p-3 text-muted-foreground max-w-[250px] truncate" title={log.reason}>
                            {log.reason}
                          </td>
                        </tr>
                      ))}
            ...
            <SectionCard title="Credit Scoring Explainability weights" description="Top variables contributing to the advisory SNV Trust Score calculation">
              <div className="space-y-3 p-3 rounded border border-border bg-surface">
                {[
                  ...
                  <div key={idx} className="flex justify-between items-center bg-card border p-2 rounded">
              ...
              <form onSubmit={handleAddOverride} className="space-y-4">
                <div className="rounded-lg bg-surface border border-border p-3 space-y-1">
                  <p className="font-bold text-foreground text-sm flex items-center gap-1.5">
                    <Activity className="size-4 text-primary animate-pulse" /> Override scoring engine
                  </p>
                  <p className="text-muted-foreground leading-relaxed text-[10px]">
                    Authorized underwriter overrides bypass default score allocations. Logged for regulatory audit.
                  </p>
                </div>
