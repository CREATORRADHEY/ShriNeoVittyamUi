...
export const Route = createFileRoute("/app/agent/leads")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Review nearby lead requests, accept files, and manage client communications.",
      },
...
function AgentLeadsPage() {
  ...
  const [chatHistory, setChatHistory] = useState([
    {
      sender: "agent",
      text: "Hello, I am reviewing your document checklist. Please prepare your unlocked bank statements.",
      time: "10:30",
    },
  ]);
  ...
  const handleAcceptLead = (id: string) => {
    ...
    setLeads((prev) => prev.map((l) => (l.id === id ? { ...l, status: "Assigned" as const } : l)));
      ...
      {isNewOrEmpty ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-border bg-card p-12 text-center">
          ...
          <p className="mt-2 max-w-[45ch] text-xs text-muted-foreground">
            There are no nearby client assistance requests in your service areas right now. We will
            alert you when a lead arrives.
          </p>
        </div>
      ) : (
          ...
          {account === "new" && (
            <div className="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4 text-amber-900 text-xs">
              ...
              <div>
                ...
                <p className="mt-1">
                  Your profile verification is pending. You can review available requests, but
                  locking leads or opening client chats requires active certification.
                </p>
              </div>
            </div>
          )}
                ...
                <tbody>
                  {leads.map((l) => (
                    ...
                    >
                      ...
                      <td className="p-3">
                        <span
                          className={`px-2 py-0.5 rounded text-[10px] font-semibold ${l.status === "Assigned" ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : l.status === "Consent Pending" ? "bg-amber-50 text-amber-700 border border-amber-200" : "bg-primary/10 text-primary border border-primary/20"}`}
                        >
                          {l.status}
                        </span>
                      </td>
                    ...
                    selectedLead.status === "Assigned" && (
                      <div className="flex gap-2">
                        ...
                        <Button
                          ...
                          className="flex items-center gap-1"
                          onClick={() =>
                            toast.success("Connecting masked telephony call via ShriNeo bridge...")
                          }
                        >
                        ...
                        <div className="space-y-2">
                          <div className="p-3 border border-border bg-card rounded flex justify-between items-center">
                            <div>
                              ...
                              <p className="text-[10px] text-muted-foreground">
                                Linked via DigiLocker on 08 Mar 2026
                              </p>
                            </div>
                          ...
                          <div className="p-3 border border-border bg-card rounded flex justify-between items-center">
                            <div>
                              ...
                              <p className="text-[10px] text-red-700 font-semibold bg-red-50 border border-red-200 rounded px-1.5 py-0.5 inline-block mt-1">
                                RE-UPLOAD REQUEST OUTSTANDING
                              </p>
                              <p className="text-[10px] text-muted-foreground mt-1">
                                Reason: PDF file was password locked.
                              </p>
                            </div>
                            <Button
                              size="xs"
                              variant="outline"
                              onClick={() => toast.success("Sent SMS reminder to client.")}
                            >
                              Remind client
                            </Button>
                          </div>
              ...
              {chatOpen && selectedLead.status === "Assigned" && (
                <div className="md:col-span-1 rounded-xl border border-border bg-card overflow-hidden flex flex-col h-[350px]">
                  ...
                  <div className="p-2.5 bg-red-50 text-red-900 border-b border-red-200 text-[9px]">
                    <strong>Warning</strong>: Never ask the client for password, PIN, CVV or OTP
                    codes.
                  </div>
                  <div className="flex-1 overflow-y-auto p-3 space-y-2 text-xs">
                    {chatHistory.map((c, i) => (
                      <div
                        key={i}
                        className={`flex flex-col ${c.sender === "agent" ? "items-end" : "items-start"}`}
                      >
                        <div
                          className={`rounded px-2.5 py-1.5 max-w-[80%] ${c.sender === "agent" ? "bg-primary text-primary-foreground" : "bg-neutral-100 text-foreground"}`}
                        >
                          {c.text}
                        </div>
