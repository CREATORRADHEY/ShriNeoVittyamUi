...
export const Route = createFileRoute("/app/admin/cms")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Manage public pages, translations, and preview publish cycles.",
      },
...
function AdminCmsPage() {
  ...
  const [pages, setPages] = useState<CmsPageItem[]>([
    {
      ...
      slug: "/loans/personal",
      category: "Product",
      hindiTranslation: "Complete",
      updatedAt: "11 Mar 2026",
      status: "Published",
    },
    {
      id: "CMS-02",
      slug: "/trust-center/security",
      category: "Compliance",
      hindiTranslation: "Complete",
    ...
    {
      id: "CMS-03",
      slug: "/blog/apr-and-fees",
      category: "Editorial",
      hindiTranslation: "Missing",
  ...
  const handlePublish = (id: string) => {
    setPages((prev) =>
      prev.map((p) => (p.id === id ? { ...p, status: "Published", updatedAt: "Just now" } : p)),
    );
    ...
    >
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard
          ...
          hint="Active on production"
        />
        <KpiCard
          label="Awaiting approval"
          value={String(pages.filter((p) => p.status === "Awaiting Approval").length)}
          hint="Compliance review required"
          tone="warning"
        />
        <KpiCard
          label="Draft files"
          value={String(pages.filter((p) => p.status === "Draft").length)}
          hint="Under active editing"
        />
        <KpiCard
          label="Missing Hindi"
          value={String(pages.filter((p) => p.hindiTranslation === "Missing").length)}
          hint="Localization gap"
          tone="warning"
        />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[2fr_1.2fr] mt-6 text-xs">
        ...
        <SectionCard
          ...
          description="Click a page row to configure publish cycle options"
        >
                  ...
                  {filteredPages.map((p) => (
                    ...
                    >
                      ...
                      <td className="p-3">
                        <StatusBadge
                          tone={
                            p.hindiTranslation === "Complete"
                              ? "success"
                              : p.hindiTranslation === "In Progress"
                                ? "warning"
                                : "error"
                          }
                        >
            ...
            <SectionCard title="Publish Cycle & Preview">
              <div className="space-y-4">
                <div className="space-y-1">
                  ...
                  <span className="font-mono text-sm font-semibold text-[#002B98]">
                    {selectedPage.slug}
                  </span>
                </div>
                ...
                <div className="border-t border-border pt-3 space-y-3">
                  ...
                  <p className="text-muted-foreground leading-relaxed">
                    Configure a temporary staging preview to review visual alignment and layout
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {selectedPage.status === "Staging Preview" && (
                      <Button
                        ...
                        onClick={() =>
                          window.open("https://preview.shrineo.in/draft-04a29", "_blank")
                        }
                      >
                ...
                <div className="border-t border-border pt-3 space-y-3">
                  ...
                  <p className="text-muted-foreground leading-relaxed">
                    Promote draft changes to the global production CDN. This action invalidates the
                    cache for all regional edge locations.
                  </p>
