...
function FindAgentPage() {
  ...
  const [chatMessages, setChatMessages] = useState([
    {
      sender: "agent",
      text: "Namaste Rohit, I am assigned to your Personal Loan draft. How can I help you today?",
      time: "11:30",
    },
  ]);
  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      setChatMessages((prev) => [
        ...
        {
          sender: "agent",
          text: "Sure, please upload the requested bank statements in our chat or directly in your Action Centre.",
          time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
        },
      ]);
    }, 1500);
      ...
      <div className="space-y-6">
        <div className="flex items-start gap-3 rounded-lg border border-blue-200 bg-blue-50 p-4 text-blue-900 text-xs">
          ...
          <div>
            ...
            <p className="mt-1">
              ShriNeo agents never charge fees. They are paid commissions by lenders. If any agent
              requests financial compensation, report them immediately.
            </p>
          </div>
        </div>
        ...
        {flowState === "search" && (
          <SectionCard
            ...
            description="Select service area and product details"
          >
              ...
              <div className="flex flex-col sm:flex-row gap-5 items-center p-2">
                ...
                <div className="text-center sm:text-left flex-1 space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Agent ID: {CANONICAL_AGENT.id} · Rating: ⭐ {CANONICAL_AGENT.rating}
                  </p>
                  <p className="text-xs text-foreground">
                    Service Area: {selectedCity} ({selectedState}) · Speaks:{" "}
                    {CANONICAL_AGENT.languages.join(", ")}
                  </p>
                </div>
            ...
            <SectionCard title="Agent File Scope & Access Permissions">
              <div className="space-y-4 text-xs">
                <p className="text-muted-foreground">
                  Under RBI fair practice codes, this agent is assigned to your draft personal loan
                  application with restricted access permissions.
                </p>
                <div className="grid gap-3 sm:grid-cols-2 rounded-lg bg-surface border border-border p-4">
                  <div>
                    ...
                    <span className="font-semibold text-foreground mt-0.5 inline-flex items-center gap-1">
                      <span className="size-2 rounded-full bg-emerald-500" /> Limited File View
                      (Read-Only)
                    </span>
                  </div>
                  <div>
                    ...
                    <span className="font-mono text-foreground font-semibold mt-0.5">
                      APP-2026-001284
                    </span>
                  </div>
                  <div>
                    ...
                    <span className="text-foreground mt-0.5 block">
                      View checklist, upload requested clarifications, track status
                    </span>
                  </div>
                  <div>
                    <span className="block text-muted-foreground text-[10px] uppercase font-bold">
                    ...
                    <span className="text-foreground mt-0.5 block">
                      19 Mar 2026 (7 days from assignment)
                    </span>
                  </div>
                </div>
                <div className="rounded-lg border border-red-200 bg-red-50/50 p-3 text-red-950 flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-red-900 mt-0.5">
                      You can revoke this agent's access at any time from your Consent Manager
                      dashboard. Revoking access will immediately disconnect them from your active
                      file.
                    </p>
                  </div>
                </div>
            ...
            <div className="w-full max-w-[500px] rounded-xl border border-border bg-card shadow-[var(--shadow-overlay)] flex flex-col h-[550px]">
              <div className="p-4 border-b border-border flex items-center justify-between bg-surface rounded-t-xl">
                ...
                <button
                  ...
                  className="rounded-full p-1 text-muted-foreground hover:bg-neutral-100"
                >
                  <X className="size-5" />
                </button>
              </div>
              ...
              <div className="p-3 bg-red-50 text-red-900 border-b border-red-200 text-[10px] flex items-start gap-1">
                <ShieldAlert className="size-4 shrink-0 mt-0.5" />
                <p>
                  <strong>Safety Warning</strong>: Never share passwords, bank PINs, credit card
                  numbers, CVVs, or OTP codes with anyone, including verified agents.
                </p>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {chatMessages.map((msg, index) => (
                  <div
                    key={index}
                    className={`flex flex-col ${msg.sender === "you" ? "items-end" : msg.sender === "system" ? "items-center" : "items-start"}`}
                  >
                    {msg.sender === "system" ? (
                      ...
                      <div
                        className={`rounded-xl px-3.5 py-2 text-xs max-w-[80%] ${msg.sender === "you" ? "bg-primary text-primary-foreground rounded-tr-none" : "bg-neutral-100 text-foreground rounded-tl-none"}`}
                      >
                        {msg.text}
                      </div>
                    )}
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    setChatMessages((prev) => [
                      ...
                      {
                        sender: "system",
                        text: "Shared Aadhaar Card from Document Vault. Status: Synced securely.",
                        time,
                      },
                    ]);
            ...
            <div className="w-full max-w-[380px] rounded-xl border border-border bg-card p-6 shadow-[var(--shadow-overlay)] text-center space-y-6">
              ...
              <div>
                ...
                <p className="text-[10px] text-emerald-600 font-mono mt-1">
                  Telephony: SYSTEM DEPENDENCY (Bridged number)
                </p>
              </div>
              ...
              <div className="rounded-lg bg-surface border border-border p-4 text-xs">
                {callState === "calling" && (
                  <p className="font-semibold text-primary animate-pulse">
                    Connecting call safely...
                  </p>
                )}
                {callState === "connected" && (
                  <p className="font-semibold text-emerald-600">Connected · Call active</p>
                )}
                {callState === "ended" && (
                  <p className="font-semibold text-muted-foreground">Call Ended</p>
                )}
                {callState === "unavailable" && (
                  <p className="font-semibold text-red-600">Agent Busy / Unavailable</p>
                )}
                {callState === "schedule-callback" && (
                  <p className="font-semibold text-primary">
                    Callback request scheduled successfully
                  </p>
                )}
                {callState === "service-unavailable" && (
                  <p className="font-semibold text-red-700">
                    ShriNeo Telecom Bridge Down (Service Unavailable)
                  </p>
                )}
              </div>
              ...
              <details className="text-left">
                ...
                <div className="grid grid-cols-2 gap-1.5 pt-2">
                  <Button size="xs" variant="outline" onClick={() => setCallState("connected")}>
                    Sim Connect
                  </Button>
                  <Button size="xs" variant="outline" onClick={() => setCallState("unavailable")}>
                    Sim Unavailable
                  </Button>
                  <Button
                    size="xs"
                    variant="outline"
                    onClick={() => setCallState("schedule-callback")}
                  >
                    Sim Callback Request
                  </Button>
                  <Button
                    size="xs"
                    variant="outline"
                    onClick={() => setCallState("service-unavailable")}
                  >
                    Sim Telemetry Error
                  </Button>
                </div>
