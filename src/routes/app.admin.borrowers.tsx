  ...
  head: () => ({
    meta: [
      { title: "Borrower Oversight — ShriNeo Capital" },
      {
        name: "description",
        content:
          "Search borrower accounts, inspect application history and act on support escalations.",
      },
  ...
  const [borrowers, setBorrowers] = useState<BorrowerAccount[]>([
    {
      ...
      deactivationRequestDate: "Today (30d cool-off)",
    },
    {
      id: "BR-481990",
      name: "Rohit Sharma",
    ...
    >
      <div className="space-y-6 text-xs">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <KpiCard
            label="Deactivation Cool-offs"
            value="1"
            tone="warning"
            hint="Pending 30-day review"
          />
                    ...
                    {filteredBorrowers.map((b) => (
                      ...
                      >
                        ...
                        <td className="p-3">
                          <StatusBadge
                            tone={
                              b.status === "Active" || b.status === "In application"
                                ? "success"
                                : b.status === "Locked"
                                  ? "error"
                                  : "warning"
                ...
                <div className="space-y-4">
                  ...
                  {selectedBorrower.status === "Deactivation Pending" && (
                    <div className="p-3 border border-amber-200 bg-amber-50 rounded text-amber-950 space-y-2">
                      <p className="font-semibold flex items-center gap-1.5">
                        <UserX className="size-4 shrink-0 text-amber-600" /> Account Deactivation
                        Request
                      </p>
                      <p className="text-[10px] text-muted-foreground">
                        Borrower requested account deletion. A mandatory 30-day cool-off window is
                        active before data purging.
                      </p>
                      <Button
                        size="xs"
                        className="w-full mt-1"
                        variant="outline"
                        onClick={() => handleCancelDeactivation(selectedBorrower.id)}
                      >
                        Cancel Deactivation
                      </Button>
                    </div>
                  ...
                  <div className="border-t border-border pt-4 space-y-2 flex flex-col">
                    <Button
                      ...
                      onClick={() => handleToggleLock(selectedBorrower.id)}
                    >
                      {selectedBorrower.status === "Locked" ? (
                    ...
                    <Button
                      ...
                      variant="outline"
                      className="flex items-center gap-1 justify-center"
                      onClick={() => handleTriggerKycReupload(selectedBorrower.id)}
                    >
